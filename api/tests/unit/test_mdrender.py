...
from apps.utils.mdrender.extensions import emojify
from apps.utils.mdrender.extensions import refresh_attachment
from apps.utils.mdrender.service import render, cache_by_sha, get_diff_of_htmls, render_and_extract
from apps.attachments.services import REFRESH_PARAM

import time

dummy_account = MagicMock()
dummy_account.id = 1
dummy_account.slug = "test"


dummy_object = MagicMock()
del dummy_object.slug
dummy_object.account = dummy_account


def test_proccessor_valid_emoji():
    result = emojify.EmojifyPreprocessor().run(["**:smile:**"])
    assert result == ["**![smile](https://api.realjournal.com/static/img/emojis/smile.png)**"]
...
def test_mentions_valid_username():
    with patch("apps.utils.mdrender.extensions.mentions.get_user_model") as get_user_model_mock:
        ...
        result = render(dummy_account, "text @hermione text")

        get_user_model_mock.return_value.objects.get.assert_called_with(
            memberships__account_id=1,
            username="hermione",
        )
        assert result == ('<p>text <a class="mention" href="https://account.realjournal.com/profile/hermione" '
...
def test_mentions_valid_username_with_points():
    with patch("apps.utils.mdrender.extensions.mentions.get_user_model") as get_user_model_mock:
        ...
        result = render(dummy_account, "text @luna.lovegood text")

        get_user_model_mock.return_value.objects.get.assert_called_with(
            memberships__account_id=1,
            username="luna.lovegood",
        )
        assert result == ('<p>text <a class="mention" href="https://account.realjournal.com/profile/luna.lovegood" '
...
def test_mentions_valid_username_with_dash():
    with patch("apps.utils.mdrender.extensions.mentions.get_user_model") as get_user_model_mock:
        ...
        result = render(dummy_account, "text @super-ginny text")

        get_user_model_mock.return_value.objects.get.assert_called_with(
            memberships__account_id=1,
            username="super-ginny",
        )
        assert result == ('<p>text <a class="mention" href="https://account.realjournal.com/profile/super-ginny" '
                          'title="Ginny Weasley">@super-ginny</a> text</p>')



def test_proccessor_invalid_type_reference():
    with patch("apps.utils.mdrender.extensions.references.get_instance_by_ref") as mock:
        instance = mock.return_value
        instance.content_type.model = "other"
        instance.content_object.subject = "test"
        result = render(dummy_account, "**#4**")
...
def test_proccessor_invalid_reference():
    with patch("apps.utils.mdrender.extensions.references.get_instance_by_ref") as mock:
        ...
        result = render(dummy_account, "**#5**")
        assert result == "<p><strong>#5</strong></p>"


def test_render_wiki_strong():
    assert render(dummy_account, "**test**") == "<p><strong>test</strong></p>"
    assert render(dummy_account, "__test__") == "<p><strong>test</strong></p>"


def test_render_wiki_italic():
    assert render(dummy_account, "*test*") == "<p><em>test</em></p>"
    assert render(dummy_account, "_test_") == "<p><em>test</em></p>"


def test_render_wiki_strike():
    assert render(dummy_account, "~~test~~") == "<p><del>test</del></p>"


def test_render_wikilink():
    expected_result = "<p><a class=\"reference wiki\" href=\"https://terminal.realjournals.com/account/test/wiki/test\" title=\"test\">test</a></p>"
    assert render(dummy_account, "[[test]]") == expected_result
...
def test_render_wikilink_2():
    expected_result = "<p><a class=\"reference wiki\" href=\"https://terminal.realjournals.com/account/test/wiki/test-page\" title=\"test page\">test page</a></p>"
    assert render(dummy_account, "[[test page]]") == expected_result


def test_render_wikilink_3():
    expected_result = "<p><a class=\"reference wiki\" href=\"https://terminal.realjournals.com/account/test/wiki/testpage\" title=\"TestPage\">TestPage</a></p>"
    assert render(dummy_account, "[[TestPage]]") == expected_result


def test_render_wikilink_with_custom_title():
    expected_result = "<p><a class=\"reference wiki\" href=\"https://terminal.realjournals.com/account/test/wiki/test\" title=\"custom\">custom</a></p>"
    assert render(dummy_account, "[[test|custom]]") == expected_result


def test_render_wikilink_slug_to_wikipages():
    expected_result = "<p><a class=\"reference wiki\" href=\"https://terminal.realjournals.com/account/test/wiki/wiki_page\" title=\"wiki page\">wiki</a></p>"
    assert render(dummy_account, "[wiki](wiki_page \"wiki page\")") == expected_result


def test_render_wikilink_relative_to_absolute():
    expected_result = "<p><a href=\"https://terminal.realjournals.com/account/test/\">test account</a></p>"
    assert render(dummy_account, "[test account](/account/test/)") == expected_result


def test_render_wikilink_obj_without_slug_absolute():
    expected_result = "<p><a href=\"https://terminal.realjournals.com/account/test/\">test account</a></p>"
    assert render(dummy_object, "[test account](/account/test/)") == expected_result


def test_render_wikilink_obj_without_slug_relative():
    expected_result = "<p><a class=\"reference wiki\" href=\"https://terminal.realjournals.com/account/test/wiki/wiki_page\">test account</a></p>"
    assert render(dummy_object, "[test account](wiki_page)") == expected_result
...
def test_render_reference_links():
    ...
    assert render(dummy_account, source) == expected_result
...
def test_render_absolute_image():
    assert render(dummy_account, "![test](/test.png)") == "<p><img alt=\"test\" src=\"/test.png\"></p>"
...
def test_render_triple_quote_code():
    ...
    assert render(dummy_account, "```python\nprint(\"test\")\n```") == expected_result
...
def test_cache_by_sha():
    ...
    result_a_1 = test_cache(dummy_account, "A" + padding)
    result_b_1 = test_cache(dummy_account, "B")
    result_a_2 = test_cache(dummy_account, "A" + padding)
    result_b_2 = test_cache(dummy_account, "B")
...
def test_render_and_extract_references():
    with patch("apps.utils.mdrender.extensions.references.get_instance_by_ref") as mock:
        instance = mock.return_value
        instance.content_type.model = "account"
        instance.content_object.subject = "test"
        (_, extracted) = render_and_extract(dummy_account, "**#1**")
...
def test_render_attachment_image(settings):
    ...
    with patch("apps.utils.mdrender.extensions.refresh_attachment.get_attachment_by_id") as mock:
        attachment = mock.return_value
        attachment.id = 42
        attachment.attached_file.url = sentinel_url

        result = render(dummy_account, md)

    assert result == expected_result
    assert mock.called is True
    mock.assert_called_with(dummy_account.id, 42)
