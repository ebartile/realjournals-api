...
def test_get_attachment_by_id(django_assert_num_queries):
    att, other_att = f.AccountAttachmentFactory(), f.AccountAttachmentFactory()
    assert att.content_object.account_id != other_att.content_object.account_id

    # Attachment not exist
    with django_assert_num_queries(1):
        assert services.get_attachment_by_id(other_att.content_object.account_id, 99999) is None

    # Attachment does not belong to an object of the account
    with django_assert_num_queries(2):
        assert services.get_attachment_by_id(other_att.content_object.account_id, att.id) is None

    # Attachment do belongs to the account
    with django_assert_num_queries(2):
        assert services.get_attachment_by_id(att.content_object.account_id, att.id) == att
